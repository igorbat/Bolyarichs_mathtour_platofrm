...
class PlayerCache:
    def __init__(self, param_for_db=DB_NAME):
        ...
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS players(user_id TEXT PRIMARY KEY, tour TEXT, fio TEXT, reggi TEXT);"
        )
        self.conn.commit()
    ...
    def set_unfixed(self, id):
        ...
        return (
            True,
            "Анкета вернулась учащемуся",
            "<@{}> {}: Ваша анкета отвергнута".format(id, self.players_storage[id].fio),
        )
    ...
    def allow(self, id, regg):
        ...
        cursor = self.conn.cursor()
        entry = (
            self.players_storage[id].user_id,
            self.players_storage[id].tour,
            self.players_storage[id].fio,
            regg,
        )
        ...
        return (
            True,
            "Игрок принят",
            "<@{}> {}: Ваша анкета принята".format(id, self.players_storage[id].fio),
        )
    ...
    def admin_change_tour(self, id, new_tour):
        ...
        return (
            "True",
            "Турнир Сменен",
            "{}: Ваш турнир сменен на {}".format(
                self.players_storage[id].fio, new_tour
            ),
        )
