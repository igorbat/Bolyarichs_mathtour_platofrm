...
class TaskCache:
    def __init__(self, param_for_db=DB_NAME):
        ...
        self.tours = {"pro": defaultdict(Task), "novice": defaultdict(Task)}

        cursor = self.conn.cursor()
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS tasks(tour TEXT, name TEXT, ans TEXT);"
        )
        self.conn.commit()
        ...
        for task in cursor.fetchall():
            ...
            if num1 > num2:
                num1, num2 = num2, num1
            self.tours[task[0]][(num1, num2)].load_from_row(
                [task[0], num1, num2, task[2]]
            )
