...
class SolutionCache:
    def __init__(self, param_for_db=DB_NAME):
        ...
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS solutions(user_id TEXT, name TEXT, answer TEXT, timee INT);"
        )
        self.conn.commit()

        cursor.execute("SELECT * FROM solutions;")  # достаём из бд все решения
        for solution in cursor.fetchall():
            self.solution_storage.append(
                [solution[0], solution[1], solution[2], solution[3]]
            )

    def new_solution(
        self, user_id, num1, num2, answer, timee=int(datetime.timestamp(datetime.now()))
    ):
